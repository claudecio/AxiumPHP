<?php
    ...
    class RequestHelper {
         ...
         */
        public static function sendJsonResponse(int $response_code = 200, string $status = 'success', string $message = '', array $data = []): void {
            // Garante que o status seja válido
            if (!in_array($status, ['success', 'error', 'fail'])) {
                $status = 'error';
            }
