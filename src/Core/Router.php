<?php
    ...
    class Router {
        ...
        public static function dispatch(): void {
            ...
            // Verifica se o tipo de requisição é válida
            ...
            // Retorna os Cabeçalhos CORS
            if ($method === 'OPTIONS' && self::$ROUTER_MODE === 'JSON') {
                // Verifica a origem da requisição
                $origin = $_SERVER['HTTP_ORIGIN'] ?? '';

                switch(true) {
                    case in_array(needle: '*', haystack: self::$ROUTER_ALLOWED_ORIGINS):
                        header(header: "Access-Control-Allow-Origin: *");
                    break;

                    case (in_array(needle: $origin, haystack: self::$ROUTER_ALLOWED_ORIGINS)):
                        header(header: "Access-Control-Allow-Origin: $origin");
                    break;

                    default:
                        if (self::$APP_SYS_MODE === 'DEV') {
                            // Em homologação: libera qualquer origem
                            header(header: "Access-Control-Allow-Origin: {$origin}");
                        } else {
                            // Em produção: bloqueia e informa
